...
def get_json(url: str, key: str) -> dict:
    req=urllib.request.Request(url,headers={"Authorization":f"Bearer {key}","User-Agent":"LLM-Persona-Followup-ZeroCost/1.2"})
...
def choose_endpoint(model_id: str, key: str, provider_name: str) -> dict:
    ...
    matches.sort(key=lambda x:(x["output_per_m"],x["input_per_m"],x["tag"])); return matches[0]
...
def main() -> int:
    ...
    provider_name=reg["privacy"]["provider_name"]; live_models={}; live_total=0.0
    for mk in sorted({arm["model"] for arm in study["arms"]}):
        ...
        live_models[mk]={"model":mid,"reasoning_metadata":reasoning_meta,"frozen_enabled_efforts":sorted(enabled),"endpoint":ep,"hard_single_pass_ceiling_usd":round(cost,6)}

    cap=float(study["study_spend_cap_usd"]); remaining=key_info.get("limit_remaining")
    report={"study_id":a.study_id,"status":"PASS_LIVE_PROJECTION" if projection else "PASS_LIVE","projection_only":projection,"launch_ready":not projection,"paid_inference_performed":False,"chat_completions_called":False,"provider_name":provider_name,"allow_fallbacks":False,"data_collection":"deny","models":live_models,"hard_single_pass_live_ceiling_usd":round(live_total,6),"study_spend_cap_usd":cap,"openrouter_key":{"is_free_tier":key_info.get("is_free_tier"),"limit":key_info.get("limit"),"limit_remaining":remaining,"usage":key_info.get("usage")},"static":static}
    if live_total>cap+1e-9: report["status"]="BLOCKED_LIVE_PRICE_PROJECTION" if projection else "BLOCKED_LIVE_PRICE"
    if not projection and remaining is not None and float(remaining)<min(live_total,cap): report["status"]="BLOCKED_BALANCE"
